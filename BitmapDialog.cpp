...
BOOL CBitmapDialog :: LoadBitmap (LPCTSTR lpszResourceName, LPCTSTR lpszFilename)
{
	...
	if (lpszResourceName != NULL)
	{
		...
		MakeWindowRgn (m_iOffsetX, m_iOffsetY);
	}
	else
	{
...
void CBitmapDialog :: MakeWindowRgn (int ix, int iy)
{
	if (!m_bTransparent)
	{
...
void CBitmapDialog::SetOffset(int a_iOffsetX, int a_iOffsetY)
{
	m_iOffsetX = a_iOffsetX;
	m_iOffsetY = a_iOffsetY;
}
